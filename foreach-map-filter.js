...
function doubleValues(arr){
    const newA = arr.map(function(el){
        return el*2;
    })
    return newA;
}
...
*/
function findAvowel(up,low,arr,obj){
    
    const filt =  arr.filter(function(El){
        return El == up || El == low;
    })
    if (filt.length !== 0 ) {obj[low] = filt.length}
...
function vowelCount(str){
    ...
    findAvowel('A','a',Arr,obj); 
    findAvowel('E','e',Arr,obj); 
    findAvowel('I','i',Arr,obj); 
    findAvowel('O','o',Arr,obj); 
    findAvowel('U','u',Arr,obj); 
...
function find(arr, searchValue) {
    for (let El in arr){
        if (El == searchValue){ return searchValue}
    }
}
...
function findInObj(arr, key, searchValue) {
    for (let El of arr){
        if (El[key] == searchValue){ return El }
    }
...
function removeVowels(str) {
    ...
    let Arr = Array.from(lower);
    
    const as = vowelList('a',Arr); 
    const es = vowelList('e',as); 
    const is = vowelList('i',es); 
    const os = vowelList('o',is); 
    const fin = vowelList('u',os); 
    let result = '';
